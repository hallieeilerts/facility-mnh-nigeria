...
#' Inputs
mypath_data <- "./data/"
mypath_results <- "./gen/monitoring/tables"
toolc_tables <- readRDS(paste0(mypath_data, "facilityMNCH-toolC.rds"))
dat <- toolc_tables$`hfe-tool-c-final`
dat_q27 <- toolc_tables$`hfe-tool-c-final-dth_review_repeat`
################################################################################

# dat %>%
#   #filter(f03 == "45-BBCH-C") %>% 
#   select(starts_with("m242")) %>% View()

# Intro -------------------------------------------------------------------

# level1-3 and g01-g03 sit OUTSIDE begin_group gave_consent
...
# Facility info / consent artifacts (inside gave_consent group. dat has been filtered to consented)
...
# Flag blank, non-numeric, or negative values.
# 8888 is the expected "not available/missing" code and is NOT flagged

flag_invalid_month <- function(x) {
  num_x <- suppressWarnings(as.numeric(x))
  is.na(x) | trimws(x) == "" | (is.na(num_x) & !is.na(x)) | (!is.na(num_x) & num_x < 0)
}

# family metadata: prefix = column prefix (e.g. "m242a1"), gate_var = the
# calculate field this family's relevant condition depends on (NA = unconditional
# field-list group, always asked), label_stub = human-readable indicator name
month_families <- tribble(
  ~prefix,    ~gate_var,      ~label_stub,
  "m242a1", NA_character_, "Facility-based maternal deaths",
  ...
  "m242a6", NA_character_, "Community-based maternal deaths",
  ...
  "m242b1", NA_character_, "Facility-based neonatal deaths",
  ...
  "m242b6", NA_character_, "Community-based neonatal deaths",
  ...
  "m242c1", NA_character_, "Facility-based stillbirths",
  ...
  "m242c6", NA_character_, "Community-based stillbirths",
...
month_check_defs <- pmap_dfr(month_families, function(prefix, gate_var, label_stub) {
  map_dfr(1:12, function(m) {
    colname <- paste0(prefix, "_m", m)
    if (!colname %in% names(dat)) return(NULL)  # skip gracefully if column isn't in this export yet
    ...
    flag <- gate & flag_invalid_month(dat[[colname]])
    cond_name <- paste0("cond_", colname)
    month_conditions[[cond_name]] <<- flag
    
    tibble(
      cond_name = cond_name,
      question  = colname,
      label     = paste0(label_stub, " - ", month_labels[m]),
      issue     = paste0(label_stub, " for ", month_labels[m],
                         " is blank, non-numeric, or negative (", colname, ")")
    )
  })
})
...
check_defs <- tribble(
  ...
  "cond_f04",       "f04",            "Assistant's position",      "Position of person assisting is blank (f04)",
  "cond_f04oth",    "f04 / f04oth",   "Assistant's position other", "Other selected for assistant's position (f04=96) but f04oth is blank",
  
  # NEW: system characteristics intro
...
var_key <- tibble::tribble(
  ...
  "KEY",         "key",
  "f03",         "observation id",
  "question",    "question number in tool (q27 rows are one summary row per death type reviewed, per submission)",
  "label",       "question label (for q27 rows: which death type -- maternal/stillbirth/neonatal/child -- this row is about)",
  "issue",       "description of issue (for q27 rows: all flagged sub-issues for that death type, concatenated)"
)
...
saveWorkbook(wb, paste0(mypath_results,"./data-queries-toolC_", format(Sys.Date(), "%Y%m%d"), ".xlsx"), overwrite = TRUE)
